...
*/
		...
		if(shMem->prEx.valid == 1 && tmp.execTime<*(shMem->pExec[shMem->prEx.pID])){
			if(kill(shMem->prEx.pID, SIGUSR1) == 0)
				kill(schID, SIGUSR2);
		}						
